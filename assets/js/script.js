...
const firebaseConfig = {
  databaseURL: "https://undangan-yuli-andi-default-rtdb.asia-southeast1.firebasedatabase.app"
};
...
function submitUcapan(status) {
  ...
  db.ref("ucapan").push(ucapan);

  document.getElementById("namaInput").value = "";
  document.getElementById("komentarInput").value = "";
}

function tampilkanUcapan(snapshot) {
  ...
  entries.forEach(([id, item]) => {
    ...
    div.innerHTML = `<strong>${item.nama} (${item.status})</strong><br>${item.komentar}`;
...
db.ref("ucapan").on("value", tampilkanUcapan);
